...
function visit_stats()
{
	...
	//$area = iconv('utf-8','gbk//ignore',$area);
	//$area=iconv('UTF-8', 'GBK//IGNORE',$area);
...
function assign_comment($id, $type, $page = 1)
{
	...
	*/
    $sql = 'SELECT c.*,u.user_img FROM ' . $GLOBALS['ecs']->table('comment') . ' as c left join ' . $GLOBALS['ecs']->table('users') . 
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['comment_id']]['user_img']     = $row['user_img'];
		$arr[$row['comment_id']]['pic1']     = $row['pic1'];
		$arr[$row['comment_id']]['pic2']     = $row['pic2'];
		$arr[$row['comment_id']]['pic3']     = $row['pic3'];
    }
    /* 取得已有回复的评论 */
    if ($ids)
    {
...
function license_info()
{
    if($GLOBALS['_CFG']['licensed'] > 0)
    {
        ...
        $license = '<a href="http://www.zhiyuanit.com/license.php?product=ecshop_b2c&url=' . urlencode($host) . '" target="_blank"
>&nbsp;&nbsp;Licensed</a>';
        return $license;
    }
    else
