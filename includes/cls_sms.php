...
class sms
{
    ...
    function send($phones,$msg,$send_date = '', $send_num = 1,$sms_type='',$version='1.0')
    {
		...
		if (substr($phones, -1)==','){$phones=substr($phones,0,-1);}
		...
		$this->errors['server_errors']['error_no'] = 3;//设置成功后删除此段
        return false;//设置成功后删除此段
		
		$msg= urlencode(mb_convert_encoding($msg,'gb2312','utf-8' )); 	  
		$file_contents = file_get_contents('http://115.28.0.54:6663/shttp.recmt?ua=huihai&pw=907144&mobile='.$phones.'&msg='.$msg.'');   
