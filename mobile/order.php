...
if (strpos($user_agent, 'MicroMessenger') === false) {
   ...
   foreach( $payment_list as $key => $value ) {
	if(in_array('wxpay',$value)) unset($payment_list[$key]);
   }	
} else {
   foreach( $payment_list as $key => $value ) {
	if(in_array('alipay',$value)) unset($payment_list[$key]);
   } 
}
...
elseif($_REQUEST['act'] = 'done')
{
    ...
    if ($payment['pay_code'] == 'balance' && $order['order_amount'] > 0)
    {
        ...
        {
			$tips = '<br><br>您的余额不足以支付整个订单<br><br><a href="javascript:history.back()" class=red>返回上一页</a>';
			$smarty->assign('footer', get_footer());
			$smarty->assign('tips', $tips);
    ...
    {
        $smarty->assign('user_name', $_SESSION['user_name']);
		//让没有登录的可以查看订单 23:18 2013-7-28
		//setcookie("ECS[order_id]", $order['order_id'], time() + 31536000, '/');
		//$tips = '订单已经生成，由于您不是会员，可在 <a href="nologin_order_info.php">临时订单</a> 查看详情!';
    }else{
		//$tips = '<a href=user.php?act=order_info&id='.$order['order_id'].' class=red><br><br>订单已经生成！<br><br>订单号：'.$order['order_sn'].'<br><br>查看 订单详情！</a>';
	}
	$tips = '订单已经生成！<b class="c6">金额：'.$total['amount_formated'].'</b>';
    $smarty->assign('footer', get_footer());
    $smarty->assign('tips', $tips);
    $smarty->display('order_done.dwt');
    exit;
