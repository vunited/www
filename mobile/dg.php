...
if (empty($_GET['encode']))
{
	...
	header("Location: dg.php?encode=$string\n");
...
else
{
	...
	$default_sort_order_type   = $_CFG['sort_order_type'] == '0' ? 'goods_id' : ($_CFG['sort_order_type'] == '1' ? 'shop_price' : 'salesnum');

	$sort = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'salesnum'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
	...
	if($_COOKIE['cookcityid']){
			$city="and s.city =".$_COOKIE['cookcityid'];
	}
	/* 获得符合条件的商品总数 */
	$sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
		"LEFT JOIN " . $GLOBALS['ecs']->table('suppliers') . " AS s ".
                    "ON s.suppliers_id = g.suppliers_id ".
		"WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $city $attr_in ".
	...
	/* 分页 */
	$url_format = "dg.php?category=$category&amp;keywords=" . urlencode(stripslashes($_REQUEST['keywords'])) . "&amp;brand=" . $_REQUEST['brand']."&amp;action=".$action."&amp;goods_type=" . $_REQUEST['goods_type'] . "&amp;sc_ds=" . $_REQUEST['sc_ds'];
	if (!empty($_REQUEST['intro']))
	{
	...
	$pager = get_pager('dg.php', $pager['search'], $count, $page, $size);
	...
	{
    $brand_lists[$key]['name']= $val;
	$brand_lists[$key]['id']= $key;
	}
	$smarty->assign('brand_list',$brand_lists);
	$smarty->assign('brands',$_REQUEST['brand']);
