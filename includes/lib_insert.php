...
function insert_cookcity($arr){
	
	//setcookie("cookcity");
	//setcookie("cookcityid");
...
function insert_city($arr){	
	
   
	$sql   = 'SELECT * FROM ' . $GLOBALS['ecs']->table('region') . " WHERE  is_show=1 and region_type=1";
	$query = $GLOBALS['db']->query($sql);
    $res = array();
    while ($row = $GLOBALS['db']->fetch_array($query))
    {
		$str.='<div style="clear:both"></div><a style="font-weight:bold">'.$row['region_name'].'</a>';
			
		$sqlx   = 'SELECT * FROM ' . $GLOBALS['ecs']->table('region') . " WHERE  is_show=1 and   parent_id=".$row['region_id'];
		$queryx = $GLOBALS['db']->query($sqlx);
		$resx = array();
		while ($rowx = $GLOBALS['db']->fetch_array($queryx))
		{
            $str.='<a href="index.php?act=setcity&id='.$rowx['region_id'].'" style="font-weight:normal">'.$rowx['region_name'].'</a>';
		}
		$str.='';
