...
$sql = 'SELECT * FROM ' . $ecs->table('template') . " where filename='index'";
$template = $db->GetAll($sql);
foreach  ($template as $key => $val)
    {		
		if(strstr($val['library'],"best")){
		$best_goodsn = $val['name'];
		}
    }

    $smarty->assign('best_goods',      get_recommend_goods('best',6));    // 推荐商品
	$smarty->assign('best_goodsn',     $best_goodsn);
...
elseif ($_REQUEST['step'] == 'consignee')
{
	...
	$bak=$_GET['bak'];
	if(!$bak){$bak=$_POST['bak'];}
	$smarty->assign('bak', $bak);
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
		...
		if($bak){$smarty->assign('real_goods_count', 1);}
    }
    else
    {
        /*
...
elseif ($_REQUEST['step'] == 'drop_consignee')
{
    ...
    {
        ecs_header("Location: flow.php?step=consignee&bak=".$bak."\n");
        exit;
    }
    else
    {
...
elseif ($_REQUEST['step'] == 'delall_goods')
{
