...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "SELECT * FROM " .$ecs->table('comment'). " WHERE comment_id = '$id'";
    $comment = $db->getRow($sql);	
	@unlink(ROOT_PATH . $comment['pic1']);
	@unlink(ROOT_PATH . $comment['pic2']);
	@unlink(ROOT_PATH . $comment['pic3']);
...
function get_comment_list()
{
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT * FROM " .$GLOBALS['ecs']->table('comment'). " WHERE id_value>0 and parent_id = 0 $where " .
