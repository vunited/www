...
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.is_new, g.is_best, g.is_hot, g.brand_id, g.goods_thumb, g.market_price, g.shop_price AS org_price, '.
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $goods_list[$row['goods_id']]['goods_names']   = $row['goods_name'];		
		$goods_list[$row['goods_id']]['goods_name']    = $GLOBALS['_CFG']['goods_name_length'] > 0 ? sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];		
        ...
        $goods_list[$row['goods_id']]['brand_id']         = $row['brand_id'];
    }
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_money, credit_line, last_login, is_validated, user_img FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    $info['user_img'] = $row['user_img'];
