...
//会员头像
    ...
    {
        $cfg['user_img'] = $profile['user_img'];
    }
	
	$mobile=$profile['other']['mobile_phone'];
    $cfg['mobile'] = $mobile;
...
function get_profile($user_id)
{
    ...
    $sql  = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
             " msn, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer, user_img ".
	...
	$info['user_img']    = isset($infos['user_img']) ? $infos['user_img'] : '';
...
function get_consignee_listd($user_id,$id)
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
...
function get_consignee_listq($user_id,$id)
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
            " WHERE user_id = '$user_id' and address_id <> '$id' LIMIT 5";
 ...
 */
function get_user_orders($user_id, $num = 10, $start = 0 , $order)
{
    /* 取得订单列表 */
    $arr    = array();

    $sql = "SELECT order_id, order_sn, order_status, shipping_name, invoice_no, shipping_status, pay_status, add_time, is_comment, " .
           "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee ".
           " FROM " .$GLOBALS['ecs']->table('order_info')." WHERE user_id = '$user_id' " ;
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
			$row['handler'] = "<a href=\"javascript:void(0)\" onclick=\"checkdd('".$GLOBALS['_LANG']['confirm_cancel']."','user.php?act=cancel_order&order_id=".$row['order_id']."')\">".$GLOBALS['_LANG']['cancel']."</a>";
        ...
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            {
				@$row['handler'] = "<a href=\"javascript:void(0)\" onclick=\"checkdd('".$GLOBALS['_LANG']['received']."','user.php?act=affirm_received&order_id=".$row['order_id']."')\">".$GLOBALS['_LANG']['received']."</a>";
        ...
        $row['order_statu'] = $row['order_status'];
 ...
 * @return  array       $order_list     订单列表
 */
function get_user_orders_back($user_id, $num = 10, $start = 0, $case)
{
    /* 取得订单列表 */
    $arr    = array();
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$goods_id=rtrim($row['goods_id'], "<br/>");
		$goodsid = explode('<br/>', $goods_id); 
		$goods=rtrim($row['goods'], "<br/>");
		$goodss = explode('<br/>', $goods); 				
		for ($i = 0; $i < count($goodss); $i++)
        {
		   $goodsss[$i]['goods_id'] = $goodsid[$i];
        }		
