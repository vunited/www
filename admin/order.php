...
elseif ($_REQUEST['act'] == 'back_ok')
{
	...
	$update = "update " .$ecs->table('order_back'). " set receve = '$receve' where back_sn='$back_sn'";
    $db->query($update);
...
elseif ($_REQUEST['act'] == 'back_money')
{
	...
	$insert = "INSERT INTO " .$ecs->table('user_account').
               " VALUES ('', '$user_id', '$_SESSION[admin_name]', '$back_money', '".gmtime()."', '".gmtime()."', '$goods', '$goods', '0', '余额支付', '1')";
	$db->query($insert);	
    // 更新会员余额数量
...
function list_back($back_sn,$order_sn,$invoice_no,$status)
{
        ...
        /* 查询 */
        $sql = "SELECT * FROM " . $GLOBALS['ecs']->table("order_back") . " $where ORDER BY add_time DESC";
