...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email', 'check_mobile','clear_history','qpassword_name', 'get_passwd_question', 'check_answer', 'oath' , 'oath_login', 'other_login', 'send_pwd_mobile');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_search', 'tuihuo', 'sub_back', 'back_detail', 'back_submit', 'order_back', 'order_back_search','back_del', 'order_detail', 'address_list', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer', 'order_comment', 'fw','edit_mobile_phone','act_edit_mobile_phone');
...
if ($action == 'default')
{
	...
	$countNoComment=$db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE `is_comment` =0 and shipping_status = 2 and user_id = '$user_id'");
    $smarty->assign('countNoComment',  $countNoComment);
...
{    
	...
	$record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('fw') ." where user_id=$user_id ".$sqls  );
    $pager  = get_pager('user.php', array('act' => $action), $record_count, $page);
	$fw = get_fw($pager['size'], $pager['start'], $sqls );	
    $smarty->assign('pager',  $pager);
    $smarty->assign('fw_list', $fw);
	
	
	$smarty->assign('action',     'fw');
    $smarty->display('user_clips.dwt');
}
elseif ($action == 'fwdel')
{
...
if ($action == 'build_qrcode') {    
	$data = $_GET['data'];
	$order_sn = $_GET['order_sn'];
	$iframe1='<iframe runat="server" src="user.php?act=build_qrcodes&data='.$data.'" width="350" height="350" frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes"></iframe>';
	$iframe2='<iframe runat="server" src="user.php?act=hq&order_sn='.$order_sn.'" width="0" height="0" frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes"></iframe>';	
	
	assign_template();	
	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('user_account') . " WHERE id = ".$_GET['order_sn'];
	$row = $db->getRow($sql);	
	$smarty->assign('amount', $row['amount']);
    $smarty->assign('iframe1', $iframe1);
	$smarty->assign('iframe2', $iframe2); 
	$smarty->display('user_wx.dwt');
...
if ($action == 'hq') { 
	...
	$id = $row['order_id'];
	$url='user.php?act=order_detail&order_id='.$id;
	
	if($status!=2){
		$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('fw') . " WHERE order_sn = '{$order_sn}'";
		$row = $db->getRow($sql);
		if($row['pay_status']==2){
		$status = 2;
		$url='user.php?act=fw';
		...
		if($row['is_paid']=='1'){
		$status = 2;
		$url='user.php?act=account_deposit';
		}
	}
	...
	if($status==2){
		/*
		echo "<script>alert('支付成功');window.parent.close();</script>";
		*/
		echo "<script>alert('支付成功');window.parent.location.href='$url';</script>";
...
elseif ($action == 'act_register')
{
    ...
    else
    {
		...
		if(empty($_SESSION['mobile_code']) or $_SESSION['mobile_code'] != $mobile_code or $_SESSION['mobile'] != $other['mobile_phone']){
			show_message("短信验证码错误！");
		}
...
elseif($action == 'oath_login')
{
	...
	if($c)
	{
		...
		if(!$access)
		{
			show_message( '第三方登陆失败' , '首页', $ecs->url() , 'error');
		}
		$c->setAccessToken($access);
		$info = $c->getMessage();
		if(!$info)
		{
			show_message('第三方登陆失败' , '首页' , $ecs->url() , 'error' , false);
		}
		if(!$info['user_id'])
			show_message('第三方登陆失败' , '首页' , $ecs->url() , 'error' , false);
...
elseif ($action == 'act_edit_profile')
{
	...
	//取得该用户手机号
	$sql_mobile = "SELECT mobile_phone FROM " . $ecs->table('users').
               " WHERE user_id = '$_SESSION[user_id]'";
	$mobile = $db->getOne($sql_mobile);
	$other['mobile_phone'] = $mobile_phone = $mobile_phone ? $mobile_phone : $mobile;
    ...
    else
    {
		...
		elseif ($user->error == ERR_MOBILE_EXISTS)
        {
            $msg = sprintf('手机已存在', $profile['other']['mobile_phone']);
        }
        else
        {
...
/* 个人资料页面 */
elseif ($action == 'edit_mobile_phone')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
...
elseif ($action == 'act_edit_mobile_phone')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
	include_once(ROOT_PATH . 'includes/cls_image.php');
	include_once(ROOT_PATH .'includes/lib_clips.php');
	...
	if (!empty($cfg['mobile']))
    {
        ...
        $sql = "SELECT user_name FROM " . $ecs->table('users').
               " WHERE mobile_phone = '$cfg[mobile]' ".
               " AND user_id != '$_SESSION[user_id]'";
        if ($db->getOne($sql, true) > 0)
        {
            show_message('该手机号已被使用,请更换!', $_LANG['back_page_up'], 'user.php?act=edit_mobile_phone', 'info');
        }
    }
	
	if(empty($_SESSION['mobile_code']) or $_SESSION['mobile_code'] != $mobile_code or $_SESSION['mobile'] != $mobile_phone){
		show_message("短信验证码错误！", $_LANG['back_page_up'], 'user.php?act=edit_mobile_phone', 'info');
	}else{
		//重至密码
		$sql="UPDATE ".$ecs->table('users'). "SET `mobile_phone`='$mobile_phone' WHERE user_id= '".$_SESSION['user_id']."'";
		$db->query($sql);
        show_message('手机号更换成功', $_LANG['back_page_up'], 'user.php?act=profile', 'info');
	}
}
...
elseif ($action == 'send_pwd_mobile')
{
    ...
    $mobile_phone = !empty($_POST['extend_field5']) ? trim($_POST['extend_field5']) : '';
	$mobile_code = isset($_POST['mobile_code']) ? trim($_POST['mobile_code']) : '';
	$password = isset($_POST['password']) ? trim($_POST['password']) : '';
	$confirm_password = isset($_POST['confirm_password']) ? trim($_POST['confirm_password']) : '';
	
	if($password != $confirm_password){
		show_message($_LANG['password_error'], $_LANG['back_page_up'], '', 'info');
	}

    //用户名和手机号是否匹配
    //$user_info = $user->get_user_info($user_name);
	$user_info = $db->getRow("SELECT * FROM " .$ecs->table('users'). " WHERE user_name = '$user_name'");

    if ($user_info && $user_info['mobile_phone'] == $mobile_phone)
    {
		if(empty($_SESSION['mobile_code']) or $_SESSION['mobile_code'] != $mobile_code or $_SESSION['mobile'] != $mobile_phone){
			show_message("短信验证码错误！", $_LANG['back_page_up'], '', 'info');
		}else{
			//重至密码
			$ecs_salt = $user_info['ec_salt'];
			$new_password = md5(md5($password).$ecs_salt);
			$sql="UPDATE ".$ecs->table('users'). "SET `password`='$new_password' WHERE user_name= '".$user_name."'";
			$db->query($sql);
            $user->logout();
...
elseif ($action == 'order_list')
{
	...
	$orders = get_user_orders($user_id, $pager['size'], $pager['start']);
		...
		if (!empty($orders))
		{
			...
			{
				$orders[$key]['total_fee'] = $val['total_fee'];
				
$sql = "select goods_name,goods_id,goods_number,goods_price from ".$ecs->table('order_goods')." where order_id=".$val['order_id']; 
$goods=$db->getAll($sql);
foreach ($goods as $keys => $vals)
{				
    $goods_thumb=$db ->getOne("select goods_thumb from ".$ecs->table("goods")." where goods_id =".$vals['goods_id']);
	$goods[$keys]['goods_thumb'] = get_image_path($vals['goods_id'], $goods_thumb, true);
	$goods[$keys]['goods_price_fmt'] = price_format($goods[$keys]['goods_price']*$goods[$keys]['goods_number']);
}
				//print_r($goods);
...
elseif ($action == 'act_moren_address')
{
        $id = intval($_GET['id']);
...
{	
	...
	$refund_pic1 = (isset($_FILES['refund_pic1']['error']) && $_FILES['refund_pic1']['error'] == 0) || (!isset($_FILES['refund_pic1']['error']) && isset($_FILES['refund_pic1']['tmp_name']) && $_FILES['refund_pic1']['tmp_name'] != 'none')
         ? $_FILES['refund_pic1'] : array();
	$refund_pic2 = (isset($_FILES['refund_pic2']['error']) && $_FILES['refund_pic2']['error'] == 0) || (!isset($_FILES['refund_pic2']['error']) && isset($_FILES['refund_pic2']['tmp_name']) && $_FILES['refund_pic2']['tmp_name'] != 'none')
         ? $_FILES['refund_pic2'] : array();
	$refund_pic3 = (isset($_FILES['refund_pic3']['error']) && $_FILES['refund_pic3']['error'] == 0) || (!isset($_FILES['refund_pic3']['error']) && isset($_FILES['refund_pic3']['tmp_name']) && $_FILES['refund_pic3']['tmp_name'] != 'none')
         ? $_FILES['refund_pic3'] : array();
	
	if ($refund_pic1)
    {
        if($_FILES['refund_pic1']['size'] / 1024 > $upload_size_limit)
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['upload_file_limit'], $upload_size_limit));
            return -1;
        }
        $img_name1 = upload_file($_FILES['refund_pic1'], 'th');
	...
	if ($refund_pic2)
    {
        if($_FILES['refund_pic2']['size'] / 1024 > $upload_size_limit)
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['upload_file_limit'], $upload_size_limit));
            return -1;
        }
        $img_name2 = upload_file($_FILES['refund_pic2'], 'th');
	...
	if ($refund_pic3)
    {
        if($_FILES['refund_pic3']['size'] / 1024 > $upload_size_limit)
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['upload_file_limit'], $upload_size_limit));
            return -1;
        }
        $img_name3 = upload_file($_FILES['refund_pic3'], 'th');
...
function get_fw($num = 10, $start = 0, $sqls='')
{
    /* 取得订单列表 */
    $arr    = array();	
