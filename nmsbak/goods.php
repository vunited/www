...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
		...
		$collect = $db->getOne("SELECT rec_id FROM " .$ecs->table('collect_goods')." WHERE user_id='$userid' and goods_id='$goods_id' ");
		$smarty->assign('collect',$collect);	
		
		$shopurl=dirname('http://'.$_SERVER['SERVER_NAME'].$_SERVER["REQUEST_URI"]);		
		$shopurl=$shopurl.'/mobile/goods.php?id='.$goods_id.'&u='.$userid;
		$smarty->assign('shopurl',$shopurl);
		
		$count = $db->getOne('SELECT COUNT(*) FROM ' .$ecs->table('comment'). " WHERE id_value = {$goods['goods_id']} AND comment_type = 0 AND status = 1 AND parent_id = 0");
        ...
        {
            $sql = 'SELECT * FROM ' . $ecs->table('suppliers') . " WHERE suppliers_id = {$goods['suppliers_id']}";
