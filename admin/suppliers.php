...
elseif ($_REQUEST['act'] == 'drop_logo')
{
    ...
    $suppliers_id = isset($_GET['id']) ? intval($_GET['id']) : 0;

    /* 取得logo名称 */
    $sql = "SELECT logo FROM " .$ecs->table('suppliers'). " WHERE suppliers_id = '$suppliers_id'";
    $logo_name = $db->getOne($sql);

    if (!empty($logo_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/logo/' .$logo_name);
        $sql = "UPDATE " .$ecs->table('suppliers'). " SET logo = '' WHERE suppliers_id = '$suppliers_id'";
        $db->query($sql);
    }
        ...
        /* 提示信息 */
        $links = array(array('href' => 'suppliers.php?act=edit&id=' . $suppliers_id,  'text' => '重新编辑该供货商'));
...
elseif (in_array($_REQUEST['act'], array('add', 'edit')))
{
    ...
    elseif ($_REQUEST['act'] == 'edit')
    {
        ...
        $suppliers['admin_list'] = $db->getAll($sql);
		$smarty->assign('country_list', get_regions());
            if ($suppliers['country'] > 0)
            {
                /* 取得省份 */
                $smarty->assign('province_list', get_regions(1, $suppliers['country']));
                if ($suppliers['province'] > 0)
                {
                    /* 取得城市 */
                    $smarty->assign('city_list', get_regions(2, $suppliers['province']));
                    if ($suppliers['city'] > 0)
                    {
                        /* 取得区域 */
                        $smarty->assign('district_list', get_regions(3, $suppliers['city']));
                    }
                }
            }
...
elseif (in_array($_REQUEST['act'], array('insert', 'update')))
{
	...
	$img_name = basename($image->upload_image($_FILES['logo'],'logo'));
    ...
    if ($_REQUEST['act'] == 'insert')
    {
        ...
        $suppliers = array('suppliers_name'   => trim($_POST['suppliers_name']),
						   ...
						   'yssj'      => trim($_POST['yssj']),
						   'gjxl'      => trim($_POST['gjxl']),
						   'dtxl'      => trim($_POST['dtxl']),
		...
		$email = mc_random(8,'abcdefghijklmnopqrstuvwxyz').'@163.com';
		
		$add_time = gmtime();
		$action_list = "goods_manage,suppliers_manage,order_view_finished";
		
		$a = get_all_by_suppliers($_REQUEST['suppliers_name']);
		
		$sql1 = "SELECT nav_list FROM " . $ecs->table('admin_user') . " WHERE action_list = 'all'";
		$row1 = $db->getRow($sql1);
		
		$sql2 = "INSERT INTO ".$ecs->table('admin_user')." (user_name, email, password, add_time, nav_list, action_list,suppliers_id, role_id,is_check) ".
		"VALUES ('$a[suppliers_name]', '$email', '" .trim(md5($_POST['password'])). "', '$add_time', '$row1[nav_list]', '$action_list','$a[suppliers_id]', '0','1')";
    ...
    if ($_REQUEST['act'] == 'update')
    {
		...
		if ($img_name){
		$suppliers['new']['logo']=trim($img_name);
		}
		...
		$add_time = gmtime();
		$action_list = "goods_manage,suppliers_manage,order_os_edit,order_ps_edit,order_ss_edit,order_view,order_view_finished,delivery_view,back_view";
		
		$a = get_all_by_suppliers($_REQUEST['suppliers_name']);
		
		if (check_username($a[suppliers_name])){//;//判读当前供货商是否为空
			
			$sql2 = "INSERT INTO ".$ecs->table('admin_user')." (user_name, email, password, add_time, nav_list, action_list,suppliers_id, role_id,is_check) ".
			"VALUES ('$a[suppliers_name]', '$email', '" .trim(md5($_POST['password'])). "', '$add_time', '', '$action_list','" . $suppliers['id'] . "', '0','1')";//acai2046 去掉顶部导航$row1[nav_list]
			
			$db->query($sql2);
		}else{
			if (!empty($_POST['password'])){
				$sql = "update ".$GLOBALS['ecs']->table('admin_user')." set password = '".md5($_REQUEST['password'])."',action_list='$action_list',nav_list='' where suppliers_id = '".$suppliers['id']."'";
				$db->query($sql);
			}
		}
...
function check_username($username) {
	$sql = "select * from ".$GLOBALS['ecs']->table('admin_user')." where user_name = '$username'";
	$row = mysql_query($sql);
	if ($row=mysql_fetch_row($row)){
		return false;
	}
	return true;
}

function get_all_by_suppliers($suppliers_name)
{
	$sql = "select * from ".$GLOBALS['ecs']->table('suppliers')." where suppliers_name = '$suppliers_name'";
    $row = mysql_query($sql);
	if($row=mysql_fetch_row($row))
	{
		$row['suppliers_id'] = $row[0];
		$row['suppliers_name'] = $row[1];
		$row['password'] = $row[2];
	}
	return $row;
}
