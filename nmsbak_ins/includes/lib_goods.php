 ...
 * @return  array
 */
function get_recommend_goods($type = '', $num= '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if (empty($type_array))
        {
            foreach($type2lib as $key => $data)
            {
                ...
                {
                    if(!$num)$num = get_library_number($data);
		...
		if($_COOKIE['cookcityid'] and $type=='best'){
			$city="and s.city =".$_COOKIE['cookcityid'];
		}
		

        //取出所有符合条件的商品数据，并将结果存入对应的推荐类型数组中
        $sql = 'SELECT g.goods_id, g.is_new, g.is_best, g.is_hot, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, g.brand_id, RAND() AS rnd " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
        ...
        $sql .= ' WHERE g.goods_id ' . db_create_in($type_merge).$city;
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
            ...
            ", g.attr_img, g.attr_thumb ". //牛模式(http://niumos.com)修改
    ...
    foreach ($res AS $row)
    {
        ...
        else
        {
            ...
            $arr['spe'][$row['attr_id']]['values'][] = array(
                                                        ...
                                                        'attr_img'        => $row['attr_img'], //牛模式(http://niumos.com)改
                                                        'attr_thumb'        => $row['attr_thumb'], //牛模式(http://niumos.com)改
