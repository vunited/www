...
function get_brands($cat = 0, $app = 'brand')
{
    ...
    foreach ($row AS $key => $val)
    {
		...
		$sql = "SELECT c.cat_id FROM ".
            $GLOBALS['ecs']->table('category'). " AS c, ".
            $GLOBALS['ecs']->table('goods') . " AS g " .
            "WHERE g.brand_id = ".$val['brand_id']." AND c.cat_id = g.cat_id ".
            "GROUP BY g.cat_id";
...
function get_pro_price($spec,$goods_id,$org_price=0)
{
	...
	$pro_price = $GLOBALS['db']->getOne("SELECT pro_price FROM " .$GLOBALS['ecs']->table('products'). " WHERE goods_attr='".$goods_attr."' and goods_id='".$goods_id."'");
	if($pro_price > 0)
		return $pro_price;
	else
		return $org_price;
...
{	  
	  ...
	  $row = $GLOBALS['db']->getRow($sql);
	  if($row['is_show']==1){
		  setcookie('cookcityid',$id, gmtime() + 86400 * 7);
	  }	  
	  setcookie('cookcity',$row['region_name'], gmtime() + 86400 * 7);
	  ecs_header("Location:./\n");
}
function getCity($ip = '')
{
	...
	if($ip == ''){
		...
		$arr = explode(',', $queryIP); 
        $queryIPs = trim($arr[0]);
		$url = 'http://int.dpool.sina.com.cn/iplookup/iplookup.php?format=json&ip='.$queryIPs; 
