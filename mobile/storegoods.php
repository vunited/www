...
if (empty($_GET['encode']))
{
	...
	header("Location: storegoods.php?encode=$string\n");
...
/*------------------------------------------------------ */
	...
	$_REQUEST['store_id']	  = !empty($_REQUEST['store_id'])	  ? intval($_REQUEST['store_id'])	  : 0;
	...
	if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'form')
	{
		...
		$adv_value['store_id']	 = $_REQUEST['store_id'];
	...
	$store_id	  = $_REQUEST['store_id']			? " AND suppliers_id = '$_REQUEST[store_id]'" : '';
	...
	$sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
		...
		"AND (( 1 " . $categories . $keywords . $brand  . $store_id . $min_price . $max_price . $intro . $outstock ." ) ".$tag_where." )";
	$count = $db->getOne($sql);
	...
	/* 分页 */
	$url_format = "storegoods.php?category=$category&amp;keywords=" . urlencode(stripslashes($_REQUEST['keywords'])) . "&amp;brand=" . $_REQUEST['brand']."&amp;store_id=" . $_REQUEST['store_id']."&amp;action=".$action."&amp;goods_type=" . $_REQUEST['goods_type'] . "&amp;sc_ds=" . $_REQUEST['sc_ds'];
	if (!empty($_REQUEST['intro']))
	{
	...
	$pager = get_pager('storegoods.php', $pager['search'], $count, $page, $size);
