...
//牛模式采集插件:http://niumos.com/(淘宝店：new-modle.taobao.com QQ：303441162)-begin
    $click_url = trim($_POST['click_url']) ;//牛模式采集插件:http://niumos.com/(淘宝店：new-modle.taobao.com QQ：303441162)添加
	$sql = "UPDATE " . $ecs->table('goods') . " SET " .
           "click_url = '$click_url' " .
...
//牛模式采集插件:http://niumos.com/(淘宝店：new-modle.taobao.com QQ：303441162)-begin
        ...
        {
            admin_priv('goods_manage');
			$url = 'shops.php?act=re_taobao&goods_id=' . $goods_id;
        ...
        {
            admin_priv('goods_manage');
			$url = 'albbshops.php?act=re_taobao&goods_id=' . $goods_id;
			//$aa = db_create_in($goods_id);
       		ecs_header("Location: $url\n");
			exit;
            //update_goods($goods_id, 'is_taobao', '0');
        ...
        elseif ($_POST['type'] == 'img_local')
        {
			...
			$url = 'shops.php?act=img_local&goods_id=' . $goods_id;
			else
				$url = 'albbshops.php?act=img_local&goods_id=' . $goods_id;
			//$aa = db_create_in($goods_id);
       		ecs_header("Location: $url\n");
			exit;
         }
        ...
        elseif ($_POST['type'] == 'get_rate')
        {
			...
			$url = 'shops.php?act=getAllRate&goods_id=' . $goods_id . '&ratenum=' . $ratenum;
       		ecs_header("Location: $url\n");
			exit;
          }
		/* 修改重量 */
        ...
        elseif ($_POST['type'] == 'translate')
        {
            admin_priv('goods_manage');
			if(file_exists("shops.php"))
			$url = 'shops.php?act=translate&goods_id=' . $goods_id;
			else
				$url = 'albbshops.php?act=translate&goods_id=' . $goods_id;
       		ecs_header("Location: $url\n");
			exit;
          }		
//牛模式采集插件:http://niumos.com/(淘宝店：new-modle.taobao.com QQ：303441162)-end
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    $product['pro_price']      = $_POST['pro_price'];//牛模式(http://niumos.com)货品价格
    ...
    foreach($product['product_sn'] as $key => $value)
    {
        ...
        /* 插入货品表 */
        $sql = "INSERT INTO " . $GLOBALS['ecs']->table('products') . " (goods_id, goods_attr, product_sn, product_number, pro_price)  VALUES ('" . $product['goods_id'] . "', '$goods_attr', '$value', '" . $product['product_number'][$key] . "','". $product['pro_price'][$key] . "')";//牛模式(http://niumos.com)保存货品价格
