...
elseif ($_REQUEST['act'] == 'show_area')
{
	...
	$sql = "SELECT * FROM " . $ecs->table('region') . " WHERE region_id = '$id'";
    $region = $db->getRow($sql);
	if($region['is_show']==1){$is_show=0;}else{$is_show=1;}	
	
	
	$sql = "update " .$ecs->table('region'). " set is_show = $is_show where region_id='$id'";
	if ($db->query($sql))
    {
        admin_log($region['region_name'], 'edit', 'area');
        $region_arr = area_list($region['parent_id']);
        $smarty->assign('region_arr',   $region_arr);
        $smarty->assign('region_type', $region['region_type']);
