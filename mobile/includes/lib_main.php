...
function get_wap_pager($num, $perpage, $curr_page, $mpurl,$pvar)
{
	...
	if($num > $perpage)
	{
		...
		$multipage .= "<div id=\"pager\" class=\"page\">";
		if($curr_page > 1)
		{
			$multipage .= "<a href=\"$mpurl${setp}${pvar}=".($curr_page-1)."\" class=\"perv_page\">上一页</a> ";
		}else{
		...
		if(($curr_page++) < $pages)
		{
			$multipage .= " <a href=\"$mpurl${setp}${pvar}=".$curr_page++."\" class=\"next_page\">下一页</a>";
		}else{
...
function cart_goods_list($type = CART_GENERAL_GOODS)
{
    $sql = "SELECT a.rec_id, a.user_id, a.goods_id, a.goods_name, a.goods_sn, a.goods_number, " .
            ...
            "a.goods_price * a.goods_number AS subtotal,b.goods_id,b.goods_thumb,b.suppliers_id " .
    ...
    foreach ($arr as $key => $value)
    {
		...
		$arr[$key]['store']                 = $value['suppliers_id'];
...
$img5='foot5';
$titleh='装灯网';
if ($url=='default'){$sty1='on';$img1='foot1_1';}
if ($url=='search'){$sty2='on';$img2='foot2_1';}
if ($url=='category'){$sty3='on';$img3='foot3_1';$titleh='分类';}
if ($url=='cart'){$sty4='on';$img4='foot4_1';$titleh='购物车';}
if ($url=='carts'){$sty4='on';$img4='foot4_1';$titleh='购物车';}
if ($url=='user'){$sty5='on';$img5='foot5_1';$titleh='合伙人';}
if ($url=='buy'){$titleh='订单提交';}
if ($url=='order'){$titleh='订单管理';}
if ($url=='goods'){$titleh='商品详情';}
if ($url=='favorites'){$titleh='我的收藏';}
if ($url=='store'){$titleh='线下门店';}
if ($url=='user'){$titleh='用户中心';}
if ($url=='xd'){$titleh='服务预约';}
if ($url=='xd_ok'){$titleh='购买成功';}

if ($url=='xx'){$titleh='线下体验';}
if ($url=='xx_ok'){$titleh='预约成功';}

$GLOBALS['smarty']->assign('sty1',	   $sty1);
$GLOBALS['smarty']->assign('sty2',	   $sty2);
$GLOBALS['smarty']->assign('sty3',	   $sty3);
$GLOBALS['smarty']->assign('sty4',	   $sty4);
$GLOBALS['smarty']->assign('sty5',	   $sty5);
$GLOBALS['smarty']->assign('img1',	   $img1);
$GLOBALS['smarty']->assign('img2',	   $img2);
$GLOBALS['smarty']->assign('img3',	   $img3);
$GLOBALS['smarty']->assign('img4',	   $img4);
$GLOBALS['smarty']->assign('img5',	   $img5);
$GLOBALS['smarty']->assign('titleh',	   $titleh);

$GLOBALS['smarty']->assign('service_phone',	   $GLOBALS['_CFG']['service_phone']);

$pcat_array = get_categories_tree(0);
$GLOBALS['smarty']->assign('pcat_array' , $pcat_array);


$GLOBALS['smarty']->assign('navigator_list',        get_navigators());  //自定义导航栏
...
//setcookie('cookcity','');
    ...
    $city=getCity();	
	$cookcity=$city['city'];	
    setcookie('cookcity',$city['city'], gmtime() + 86400 * 7);		
	$sql   = 'SELECT region_id FROM ' . $GLOBALS['ecs']->table('region') . " WHERE  region_name = '".$cookcity."' and is_show=1";
	$row = $GLOBALS['db']->GetOne($sql);
	if($row){setcookie('cookcityid',$row, gmtime() + 86400 * 7);}
    }else{
	$cookcity=$_COOKIE['cookcity'];
	}
	$GLOBALS['smarty']->assign('cookcity',$cookcity);  //自定义导航栏
	


			
		$sqlx   = 'SELECT * FROM ' . $GLOBALS['ecs']->table('region') . " WHERE  is_show=1 and region_type=2 order by convert(region_name using gbk) asc";
		$queryx = $GLOBALS['db']->query($sqlx);
		$resx = array();
		while ($rowx = $GLOBALS['db']->fetch_array($queryx))
		{
            $str.='<a href="index.php?act=setcity&id='.$rowx['region_id'].'">'.$rowx['region_name'].'</a>';
		}

			

	$GLOBALS['smarty']->assign('city',$str);  //自定义导航栏
 ...
 * @return  array         列表
 */
function get_navigators($ctype = '', $catlist = array())
{
