...
if ($_REQUEST['act'] == 'setup')
{
    ...
    $sql = "SELECT region, library, sort_order, id, number, type, name FROM ".$ecs->table('template') ." ".
    ...
    while ($row= $db->FetchRow($rc))
    {
        ...
        {
            /* 动态内容 */
            $db_dyna_libs[$row['region']][$row['library']][] = array('id' => $row['id'], 'number' => $row['number'], 'type' => $row['type'], 'name' => $row['name']);
        }
        else
        {
            /* 固定内容 */
            ...
            if (isset($lib))
            {
				...
				$temp_options[$lib]['name'] = $row['name'];
            }
        }
    }
    ...
    foreach ($_POST['regions'] AS $key => $val)
    {
        ...
        if (!in_array($key, $GLOBALS['dyna_libs']) AND (isset($_POST['display'][$key]) AND $_POST['display'][$key] == 1 OR $number > 0))
        {
            $sql = "INSERT INTO " .$ecs->table('template').
                        "(theme, filename, region, library, sort_order, number, name)".
                    " VALUES ".
                        "('$curr_template', '$_POST[template_file]', '$val', '".$_POST['map'][$key]."', '" . @$_POST['sort_order'][$key] . "', '$number', '".$_POST['name'][$key]."')";
            $db->query($sql);
        }
    }
