...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price ".
           ...
           "WHERE (a.cat_id = " . intval($cat_id) ." AND v.attr_value IS NOT NULL AND a.attr_values IS NOT NULL AND a.sort_order = 200 OR a.cat_id = 0 ) or (a.cat_id = " . intval($cat_id) ." AND a.sort_order <> 200 OR a.cat_id = 0 )". 
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
                    //牛模式采集插件:http://niumos.com/(淘宝店：new-modle.taobao.com QQ：303441162)-begin
" num_iid, goods_thumb, click_url, original_img, commission_num, is_taobao, shop_title, nick, goods_weight, NOT LOCATE('http',original_img) as r_img," .
//牛模式采集插件:http://niumos.com/(淘宝店：new-modle.taobao.com QQ：303441162)-end
...
function product_list($goods_id, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        //牛模式(http://niumos.com)增加：属性价格pro_price//
        $sql = "SELECT product_id, pro_price, goods_id, goods_attr, product_sn, product_number
